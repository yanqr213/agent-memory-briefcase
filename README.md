...
- doctor 支持 Markdown / JSON / SARIF / PR comment 输出，可接入 GitHub Actions、Code Scanning 和 PR 评论
- `--output` 自动创建父目录
...
agent-memory-briefcase doctor --format sarif --output build/agent-doctor.sarif
agent-memory-briefcase doctor --format pr-comment --output build/agent-doctor-comment.md
```
...
如果希望把结果接入 GitHub Code Scanning，生成 SARIF：

```bash
agent-memory-briefcase doctor \
  --format sarif \
  --output build/agent-doctor.sarif
```

如果希望把结果写入 GitHub Actions step summary 或 PR 评论，生成稳定 Markdown：

```bash
agent-memory-briefcase doctor \
  --format pr-comment \
  --output build/agent-doctor-comment.md
```

`pr-comment` 输出包含稳定标记 `<!-- agent-memory-briefcase doctor -->`，便于后续自动化复用同一条评论。
...
python -m agent_memory_briefcase doctor --root examples/demo_bundle --format sarif --output build/agent-doctor.sarif
python -m agent_memory_briefcase doctor --root examples/demo_bundle --format pr-comment --output build/agent-doctor-comment.md
...
- name: Agent memory doctor
  run: |
    python -m agent_memory_briefcase doctor --root . --check warning --output build/agent-doctor.md
    cat build/agent-doctor.md >> "$GITHUB_STEP_SUMMARY"
```

SARIF 上传示例：

```yaml
- name: Agent memory doctor SARIF
  run: python -m agent_memory_briefcase doctor --root . --format sarif --output build/agent-doctor.sarif
- uses: github/codeql-action/upload-sarif@v3
  with:
    sarif_file: build/agent-doctor.sarif
```
...
Use `doctor` before an agent handoff or CI run to decide whether the bundle is ready, needs attention, or is blocked by structural errors. Use SARIF for GitHub Code Scanning and `pr-comment` for GitHub step summaries or reusable PR comments.

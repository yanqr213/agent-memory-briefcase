...
- lint / check 可以在 CI 中拦截过期记忆、缺失栏目、陈旧测试证据。
...
- 子命令：`init`、`add-decision`、`add-session`、`brief`、`lint`、`export`、`check`
- 项目 profile 管理
...
- 先执行 `agent-memory-briefcase check --check warning`
- 修正过期 ownership / test evidence
- 再执行 `brief`

这样得到的 brief 更稳定，减少 agent 基于过期信息推断。
...
6. Export a richer new-thread handoff with `export --format handoff`.
7. Enforce freshness with `lint` or `check` in CI.
